...
module.exports = async (req, res) => {
  ...
  jwt.verify(verificationToken, process.env.JWT_SECRET, async (err, decoded) => {
    if (err)
      return res.status(409).json({ status: false, message: 'Token is invalid or expired.' });
    /** token validation */
    if (decoded.token_type !== 'account-verification')
      return res.status(409).json({ status: false, message: 'Token is invalid or expired.' });
    ...
    if (user.otp.toString() !== otp.toString())
      return res.status(409).json({ status: false, message: 'Entered OTP is invalid.' });
    await knex('user_master')
      .update({ is_confirmed: true, otp: null, active_step: 'claim-nft' })
      .where('id', '=', decoded.me)
      .returning('id');
